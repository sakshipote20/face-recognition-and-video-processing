...
document.addEventListener('DOMContentLoaded', function() {
    ...
    uploadForm.addEventListener('submit', function(event) {
        ...
        .then(data => {
            if (data.processing_complete && data.unique_videos) {
                loadingSpinner.style.display = 'none';
                updateVideoContainer(data.unique_videos, data.zip_filename);
                videoContainer.style.display = 'block';
            } else {
                console.error('Processing failed or no videos found.');
            }
...
// function updateVideoContainer(uniqueVideos, zipFilename) {
...
//         downloadButton.href = '/download_filtered_dataset';
//         downloadButton.textContent = 'Download Filtered Dataset';
//         downloadButton.classList.add('btn', 'btn-success', 'mt-3');
//         downloadButton.download = true;
//         videoListElement.appendChild(downloadButton);
//     } else {
//         const h3Element = document.createElement('h3');
...
function updateVideoContainer(uniqueVideos, zipFilename) {
    ...
    if (uniqueVideos && uniqueVideos.length > 0) {
        ...
        // Add a download button
        const downloadButton = document.createElement('a');
        downloadButton.href = '/download_filtered_dataset';  // URL to download the zip file
        downloadButton.textContent = 'Download Filtered Dataset'; // Button text
        downloadButton.classList.add('btn', 'btn-success', 'mt-3'); // Styling
        downloadButton.download = true; // This ensures the file will be downloaded
        videoListElement.appendChild(downloadButton);
    } else {
        const h3Element = document.createElement('h3');
        h3Element.textContent = 'No unique videos found';

...
def index():
    ...
    return render_template("index1.html", unique_videos=None)
...
def upload_folder():
    if request.method == "POST":
        ...
        unique_videos,zip_filename = process_uploaded_folder(folder_path)
        
        # Return a JSON response with processing status and unique videos
        return jsonify({'processing_complete': True, 'unique_videos': unique_videos, 'zip_filename': zip_filename})
    
    # If the request method is not POST, return an empty JSON response
    return jsonify({})

@app.route('/download_filtered_dataset')
def download_filtered_dataset():
    zip_filename = "filtered_dataset.zip"
    return send_from_directory(directory=os.getcwd(), filename=zip_filename, as_attachment=True)
...
def process_uploaded_folder(folder_path):
    ...
    subprocess.run(["python", "exp.py", "--encodings", "encodings.pickle", "--input", folder_path])
    dataset_folder = "dataset"
    unique_videos = os.listdir(dataset_folder)
    zip_filename = "filtered_dataset.zip"
    zip_filepath = os.path.join(os.getcwd(), zip_filename)
    shutil.make_archive("filtered_dataset", "zip", dataset_folder)
    return unique_videos,zip_filename
